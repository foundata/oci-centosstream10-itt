...
[CentOS Stream 10](https://www.centos.org/centos10/)  for integration testing.
...
2. Clone or pull the latest changes from the [`foundata/oci-centosstream10-itt` git repository](https://github.com/foundata/oci-centosstream10-itt).
3. Change into the directory and execute the [build command](https://docs.podman.io/en/latest/markdown/podman-build.1.html):
   ```bash
   podman build -t centosstream10-itt .
   ```
...
2. Use the image you built earlier or pull the image from a registry:
   - [Quay](https://quay.io/repository/foundata/centosstream10-itt):
     ...
     ```
   - [Docker Hub](https://hub.docker.com/r/foundata/centosstream10-itt):
     ```bash
     podman pull docker.io/foundata/centosstream10-itt:latest
     ```
3. Run a container from the image:
   ```bash
   podman run --detach --systemd=always centosstream10-itt:latest "/usr/sbin/init"
   ```
